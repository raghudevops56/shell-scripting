...
INSTANCE_CREATE() {
  ...
  aws ec2 run-instances --launch-template LaunchTemplateId=${LID},Version=${LVER}  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${COMPONENT}}]" | jq .Reservations[].Instances[].PrivateIpAddress |xargs -n1
  sleep 10
  DNS_UPDATE
}
